...
def condition_actuelles(main_frame):
    ...
    #  Intégration de simulation_conditions dans le frame top/gauche
    simulation_condition = simulation_conditions()

    for i, (param, value) in enumerate(simulation_condition.items()):
        unit = {"Température": " °C", "Humidité": " %", "CO2": " ppm", "Luminosité": " lux"}.get(param, "")
...
def create_jauge(bottom_frame):
   ...
   #
    simulation_condition = simulation_conditions()
    jauge(gauges_frame, "Température", simulation_condition["Température"], 10, 40, COLORS["temp"], "°C")  #<-------------- # Appel de la fonction jauge() pour créer une jauge 
    jauge(gauges_frame, "Humidité", simulation_condition["Humidité"], 10, 100, COLORS["humidity"], "%")                      # La fonction jauge() est définie avec les paramètres suivants :
    jauge(gauges_frame, "CO2", simulation_condition["CO2"], 111, 1100, COLORS["co2"], "ppm")                                 # frame : Le conteneur parent dans lequel la jauge sera placée (ici, gauges_frame)
